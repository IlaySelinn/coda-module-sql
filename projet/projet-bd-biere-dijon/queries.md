...
SELECT b.nom, MIN(p.prix) as prix_min_ipa
FROM bar b JOIN prix p ON b.id_bar=p.id_bar
JOIN biere bi ON p.id_biere=bi.id_biere
WHERE bi.type='IPA' GROUP BY b.id_bar ORDER BY prix_min_ipa LIMIT 3;
...
WHERE NOT EXISTS (
SELECT 1 FROM prix p JOIN biere bi ON p.id_biere=bi.id_biere
WHERE p.id_bar=b.id_bar AND p.prix<6
);
